...
import { SetStatus } from '../../../common/src/intents';
...
export default {
  name: 'SetStatus',
  intentName: 'SetStatus',
  waterfall: [
    (session, args, next) => {
      azureLog('LUIS found SetStatus');
    ...
    (session, results) => {
      ...
      async function queryRns() {
        let intentResponse = await SetStatus(session.dialogData.request);
